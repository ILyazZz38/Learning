...
Ext.define('B4.form.SelectField', {
    ...
    requires: ['Ext.grid.Panel', 'B4.ux.grid.plugin.HeaderFilters'],
    ...
    */
    ...
    constructor: function () {
        ...
        me.addEvents(
            ...
            /**
             * @event fieldcleared
             * Срабатывает после создания очистки поля
             * @param {Ext.Component} this
             */
            'fieldcleared'
        );
    ...
    initComponent: function () {
        ...
        if (store) {
            ...
            me.store = me.getStoreByEnum(me.enumName);
        }
        else {
    ...
    getStoreByEnum: function (enumName) {
        var enumCls = null;
        ...
        var metaItems = enumCls.getItems();
        var enumItems = [];
        Ext.each([], function (e) {
            var value = null;
            if (Ext.isNumber(e)) {
                value = e;
            } else if (Ext.isNumeric(e)) {
                value = parseInt(e);
            } else if (Ext.isString(e)) {
                var meta = enumCls.Meta[e];
                if (meta && Ext.isNumber(meta.Value))
                    value = meta.Value;
            }
            if (Ext.isNumber(value) && Ext.Array.some(metaItems, function (x) {
                return x[0] == value;
            })) {
                enumItems.push(value);
            }
        });

        return enumCls.getStore(enumItems);
    },
    ...
    onSelectValue: function () {
        var me = this,
            ...
            elNum = rec.length == 0 ? 0 : rec.length - 1;
        ...
        me.fireEvent('valueselected', me, me.getValue());
    ...
    getValue: function () {
        ...
        if (Ext.isArray(me.value)) {
            return Ext.Array.map(me.value, function (data) {
                return me.isGetOnlyIdProperty ? data[me.idProperty] : data;
            });
        }
    ...
    //получить сериализованное содержимое селекфилда: 
    //возвращает null если нет выбранных элементов
